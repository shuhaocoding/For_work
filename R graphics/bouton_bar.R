...
# data input
df <- read_excel("StaeWBKD-type_Is.xlsx")
stp = 20
...
bar <- function(df, stp, yname) {
  p <- ggplot(data=df, aes_string(y=names(df)[2], x=names(df)[1])) + 
    ...
    scale_x_discrete(limits = c("CSpA", "MgdbpA"), labels = c("Control", "Stae CRISPRi")) + # need change
    geom_errorbar(aes_string(ymin=names(df)[3], ymax=names(df)[4]), width=.2, size=1)
  ggsave(p, file="Trial.pdf", width=10, height=19, units="cm")
  p}
