...
# dot plot
# data input
df <- read_excel("StaeKO-satellite.xlsx")
yname = "Satellite boutons"
stp = 5
ylimit = stp*ceiling(max(df[2])/stp)
xlimit = c("CSW", "DfM")
xlabel = c("Control", "Stae KO")
mycolors = c("#737373", "#006d2c")
myfills = c("#d9d9d9", "#c7e9c0")
#c("#636363", "#006d2c")
#c("#bdbdbd", "#74c476")
#c("#969696", "#31a354")
...
dot <- function(df, yname, stp, ylimit, xlimit, xlabel, mycolors, myfills, SEM) {
  p <- ggplot(data = df, aes_string(y=names(df)[2], x=names(df)[1], colour=names(df)[1], fill = names(df)[1])) +
    stat_summary(fun = "mean", geom = "bar", width = .6, size = 2) +
    geom_quasirandom(method = "tukeyDense", size = 3.2, width = .3) +
    stat_summary(fun.data = SEM, geom="errorbar", width = .3, size = 2) +
    ...
    ggsave(p, file="test.pdf", width=10, height=19, units="cm")
p}
