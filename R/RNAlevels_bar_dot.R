...
# data input
df <- read_excel("darc1 copia in sta WB KD in BWM.xlsx")
df2 <- read_excel("darc1 copia in sta WB KD in BWM_rd.xlsx")
df$Sample <- factor(df$Sample, levels = c("Control", "Stae Ci"))
df2$Sample <- factor(df2$Sample, levels = c("Control", "Stae Ci"))
...
bardot <- function(df, df2, yname, stp, ylimit, xlimit, xlabel, myfills, mycols) {
  p <- ggplot() + 
    ...
    scale_y_continuous(name=yname, limits=c(0, 3), breaks=seq(0,3,stp), expand=expansion(mult = c(0, .03))) + 
    scale_x_discrete(limits = xlimit, labels = xlabel) +
    theme_classic() +
    theme(aspect.ratio=1.26,
  ...
  ggsave(p, file="test.pdf", width=22, height=21, units="cm") 
  p}
